...
interface InputNcmProps {
    onChange: (value: Mercadoria | null) => void
    label?: string
    placeholder?: string
    required?: boolean
}

export default function InputNcm({
    onChange,
    label = "Defina a mercadoria (NCM)",
    placeholder = "Digite o nome do Mercadoria",
    required = false,
}: InputNcmProps) {
    ...
    useEffect(() => {
        const searchMercadoriaes = async () => {
            ...
            try {
                ...
                console.error("Erro ao buscar Mercadoriaes", err)
                setSuggestions([])
            } finally {
                setIsLoading(false)
            }
    ...
    const handleSelectMercadoria = (Mercadoria: Mercadoria) => {
        ...
        console.log("Mercadoria válido, atualizando estados...")
        ...
        setHighlightedIndex(-1)
        setIsOpen(false)
        console.log("Mercadoria selecionado com sucesso:", Mercadoria)
    }
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedMercadoria && newValue !== selectedMercadoria.descricao) {
            console.log("Input diferente do Mercadoria selecionado, limpando seleção")
            setSelectedMercadoria(null)
            onChange(null)
        }
    }
        ...
        switch (e.key) {
            ...
            case "Enter":
                ...
                if (highlightedIndex >= 0 && highlightedIndex < suggestions.length) {
                    const MercadoriaSelecionado = suggestions[highlightedIndex]
                    console.log("Mercadoria selecionado por teclado:", MercadoriaSelecionado)
                    handleSelectMercadoria(MercadoriaSelecionado)
                }
                break
            case "Escape":
                e.preventDefault()
                ...
                {isOpen && suggestions.length > 0 && (
                    <div className="absolute z-50 w-full mt-1 rounded-md border border-gray-200 bg-white shadow-md">
                        <div className="max-h-60 overflow-auto p-1">
                            {suggestions.map((mercadoria: Mercadoria, index) => (
                                <div
                                    key={mercadoria.id_ncm}
                                    className={`
                    flex items-center px-2 py-1.5 text-sm rounded-sm relative cursor-default select-none
                    ${index === highlightedIndex ? "bg-gray-100" : ""}
                    ${selectedMercadoria?.id_ncm === mercadoria.id_ncm ? "font-medium" : ""}
                    hover:bg-gray-100
                  `}
                                    onClick={() => handleSelectMercadoria(mercadoria)}
                                    onMouseEnter={() => setHighlightedIndex(index)}
                                >
                                    {/* Check Icon */}
                                    <svg
                                        className={`mr-2 h-4 w-4 ${selectedMercadoria?.id_ncm === mercadoria.id_ncm ? "opacity-100" : "opacity-0"}`}
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="24"
                                        height="24"
                                        viewBox="0 0 24 24"
                                    ...
                                    <span>{mercadoria.descricao}</span>
                                </div>
                            ))}
                        </div>
                    </div>
